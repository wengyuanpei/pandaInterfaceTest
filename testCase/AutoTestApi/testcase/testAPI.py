...
class Demo_API(unittest.TestCase):
    """demo系统"""
    def setUp(self):
        #requests 会话 res = self.session.get(url,params=params,data=data,**kwargs)
    ...
    def test_api(self,data):
       ...
       # 发送请求
        ...
        # 获取服务端返回的值
        self.result = re.json()['code']
        print("页面返回信息：%s" % self.result)
        ...
        # self.assertEqual(self.result['code'], readData_code, "返回实际结果是->:%s" % self.result['status'])
        # self.assertEqual(self.result['code'], readData_msg, "返回实际结果是->:%s" % self.result['message'])
